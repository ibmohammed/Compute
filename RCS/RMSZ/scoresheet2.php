...
$sqlm = mysql_query("SELECT * FROM `course` WHERE Programme = '$programme' && semester = '$smtr' && sessions  = '$ses'") or die(mysql_error()); 
echo "<h2> Semester:  ".$smtr."</h2>";?>
  ...
  <tr>
    <td><?php echo $sn;?></td>
    <td><a href="javascript:void(0);" name="Chind window" title="child title" onclick='window.open("attend2.php?semester=<?php echo $smtr."& year=".$year."& session=".$ses."& courseid=".$programme."& ccode=".$rowss['code']."& ctitle=".$rowss['title'];?>","Rating","width = 1000, height = 600,0,status = 1,")';><?php echo $rowss['code'];?></a></td>
