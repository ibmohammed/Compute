...
$mm = mysql_query("UPDATE `consultdbsnw`.`studentsnm` SET `stat` = '2' WHERE `studentsnm`.`matno`='$matno'") or die(mysql_error());
 //update result
$query=mysql_query("UPDATE  `consultdbsnw`.`results` SET  `code`='$code',`unit`='$unit',`score` =  '$score',`grade` =  '$grade1',`points`='$point',`stat`='2' WHERE  `results`.`sn` ='$sn'")or die(mysql_error());
// keep old result
$oldres= mysql_query("INSERT INTO `consultdbsnw`.`coresult` (`name`, `matric_no`, `code`, `unit`, `score`, `grade`, `points`, `programme`, `semester`, `session`, `stat`,`year`) VALUES ('$name', '$matno', '$code1', '$unit1', '$scores', '$grade11', '$point', '$programme', '$semester', '$session', 'co','$year')")or die(mysql_error());
}
