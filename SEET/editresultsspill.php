...
?>
	...
	$sql=mysql_query("SELECT * FROM `studentsnm` WHERE matno='$matno'") or die(mysql_error());
	$row=mysql_fetch_array($sql);

	
	?>
	
	<table border = "0" style = "width:700px; font-size:10px;">
  <tr>
    <td style="width: 329px"><form id="form1" name = "form1" method="post" action="">
      <table border="0">
        <tr>
          <td ><span style="font-weight: bold">Name:</span></td>
          <td style="width: 248px" ><input name="name" type="text" id="name" value="<?php echo $row['names'];?>" size="40" />
          </td>
        </tr>
        <tr>
          <td ><span style="font-weight: bold">MatricNo:</span></td>
          <td style="width: 248px" ><input name="matno" type="text" id="matno"  value="<?php echo $row['matno'];?>"/>
          </td>
        </tr>
      </table>
        <table border="0">
...
<table width="80%" align="center">
  <tr>
    <td><form action="" method="post" name="grade" id="grade">
      <strong>EDIT SPILL OVER  RESULTS</strong>
      <table style="text-align: left;color:blue;">
        <tr>
          ...
          <td ><span style="font-weight: bold">
            ...
            $queri = mysql_query("SELECT * FROM `dept` WHERE prog = '$departmentcode' && dep LIKE '%National Diploma%'") or die(mysql_error());
